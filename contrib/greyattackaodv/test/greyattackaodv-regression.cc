...
#include "greyattackaodv-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/greyattackaodv-helper.h"
 ...
 *
 * \brief greyattackaodv regression test suite
 */
class greyattackaodvRegressionTestSuite : public TestSuite
{
  public:
    greyattackaodvRegressionTestSuite()
        : TestSuite("routing-greyattackaodv-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("greyattackaodv-chain-regression-test"), TestCase::QUICK);
...
} g_greyattackaodvRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("greyattackaodv chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & greyattackaodv
    greyattackaodvHelper greyattackaodv; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(greyattackaodv);
    ...
    streamsUsed += greyattackaodv.AssignStreams(*m_nodes, streamsUsed);
    // greyattackaodv uses m_size more streams
