 ...
 */
#include "ns3/greyattackaodv-neighbor.h"
#include "ns3/greyattackaodv-packet.h"
#include "ns3/greyattackaodv-rqueue.h"
#include "ns3/greyattackaodv-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("greyattackaodv TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(greyattack_aodvTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), greyattackaodvTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(greyattack_aodvTYPE_RREQ);
...
{
    RrepAckHeaderTest()
        : TestCase("greyattackaodv RREP-ACK")
    {
    }
 ...
 * \ingroup greyattackaodv-test
 *
 * \brief Unit test for greyattackaodv routing table entry
 */
...
struct greyattackaodvRqueueTest : public TestCase
{
    greyattackaodvRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
    {
    }
...
void
greyattackaodvRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&greyattackaodvRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&greyattackaodvRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &greyattackaodvRqueueTest::CheckTimeout, this);
...
void
greyattackaodvRqueueTest::CheckSizeLimit()
{
...
void
greyattackaodvRqueueTest::CheckTimeout()
{
 ...
 * \brief Unit test for greyattackaodv routing table entry
 */
struct greyattackaodvRtableEntryTest : public TestCase
{
    greyattackaodvRtableEntryTest()
 ...
 * \ingroup greyattackaodv-test
 *
 * \brief Unit test for greyattackaodv routing table
 */
struct greyattackaodvRtableTest : public TestCase
{
    greyattackaodvRtableTest()
        : TestCase("Rtable")
    {
    }
 ...
 * \ingroup greyattackaodv-test
 *
 * \brief greyattackaodv test suite
 */
class greyattackaodvTestSuite : public TestSuite
{
  public:
    greyattackaodvTestSuite()
        : TestSuite("routing-greyattackaodv", UNIT)
    {
        ...
        AddTestCase(new greyattackaodvRqueueTest, TestCase::QUICK);
        AddTestCase(new greyattackaodvRtableEntryTest, TestCase::QUICK);
        AddTestCase(new greyattackaodvRtableTest, TestCase::QUICK);
    }
} g_greyattackaodvTestSuite; ///< the test suite
