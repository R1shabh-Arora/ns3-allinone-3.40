...
class RoutingExperiment
{
  ...
  private:
    ...
    int m_nSinks{5};                                             //!< Number of sink nodes.      Destination for the data packets
    ...
    bool m_flowMonitor{true};                                //!< Enable FlowMonitor.           provides detailed statistics about network flow, such as the number of transmitted packets, transmission rate
};
...
RoutingExperiment::Run()
{
    ...
    pos.Set("X", StringValue("ns3::UniformRandomVariable[Min=0.0|Max=200.0]")); // Adjusted area size
    pos.Set("Y", StringValue("ns3::UniformRandomVariable[Min=0.0|Max=1000.0]")); // Adjusted area size
